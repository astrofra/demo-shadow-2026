...
uniform vec4 uTreeWind; // x: amplitude, y: radial frequency, z: wind speed, w: trunk radius
...
vec3 ApplyTreeWind(vec3 local_pos, float time_sec) {
	float radial_dist = length(local_pos.xz);
	float safe_radial_dist = max(radial_dist, 1e-4);
	vec2 radial_dir = local_pos.xz / safe_radial_dist;

	float radial_mask = smoothstep(uTreeWind.w, max(uTreeWind.w * 2.5, uTreeWind.w + 1e-4), radial_dist);
	float height_mask = clamp(local_pos.y / 1.5, 0.0, 1.0);
	float foliage_mask = radial_mask * height_mask;

	float phase = radial_dist * uTreeWind.y - time_sec * uTreeWind.z;
	float secondary_phase = local_pos.y * (uTreeWind.y * 0.35) + time_sec * (uTreeWind.z * 1.37);
	float wave = sin(phase) * cos(secondary_phase);
	float offset = wave * uTreeWind.x * foliage_mask;

	local_pos.x += radial_dir.x * offset;
	local_pos.z += radial_dir.y * offset;
	local_pos.y += offset * 0.18;
...
void main() {
	// position
	vec4 vtx = vec4(ApplyTreeWind(a_position, uClock.x), 1.0);
